/*
 * JS中的内存分为堆内存和栈内存
 *	堆内存： 存储引用数据类型值（对象：键值对 函数：字符串）
 *  栈内存： 提供JS代码执行环境和存储基本类型值
 * 
 *  [堆内存释放]
 *	  让所有引用堆内存地址的变量赋值未null即可（没有变量占用这个浏览器会在空闲的时候把它释放掉）
 *  [栈内存释放]
 *    一般情况下，当函数执行完成，所形成的私有作用域（栈内存）都会自动释放掉（在栈内存中存储的值也会被释放掉），但是也有特殊不销毁的情况：
...
fn(2)();  //=>和上面的步骤一样，都先把FN执行，把FN执行后在次执行F


/*
* 在和其他值进行运算的时候有一些u区别
i++; 自身累加1，（先拿原有值进行元素，运算结束后，本身累加1）
